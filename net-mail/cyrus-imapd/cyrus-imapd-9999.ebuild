...
if [ "$PV" -eq 9999 ];then
	inherit git-r3
	EGIT_REPO_URI="https://github.com/cyrusimap/${PN}.git"
else
...
IUSE="afs backup calalarm caps clamav http kerberos ldap \
	mysql nntp pam pcre +pcre2 perl postgres replication +server \
	sieve sqlite ssl static-libs tcpd test xapian"
RESTRICT="!test? ( test )"
