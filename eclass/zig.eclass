...
zig_src_prepare() {
	...
	find "${WORKDIR}" -mindepth 1 -maxdepth 1 -type d -print| while read -r srcdir; do
		if [ -f "${srcdir}/build.zig.zon" ]; then
			hash=$(zig-hash "${srcdir}/build.zig.zon" || die)
			einfo "zig.eclass: linking $hash to ${srcdir}"
			ln -s "${srcdir}" "${EZIG_SYS_SRC}/p/$hash" || die
		elif [ -f "${srcdir}/build.zig" ]; then
			hash=$(zig fetch --global-cache-dir "${EZIG_SYS_SRC}" "${srcdir}" || die)
			einfo "zig.eclass: copied ${srcdir} to $hash"
		fi
	done
}
